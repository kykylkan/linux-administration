...
State backend має окремий lifecycle у `bootstrap/`, який використовує
обов'язковий модуль `modules/s3-backend`. `backend.tf` використовує partial
backend configuration: конкретні S3/DynamoDB names передаються в
